...
| หัวหน้างาน | คำขอลาของฉัน, คำขอยกเลิกใบลา, คำขอลาของฉันที่รออนุมัติ, คำขอลาของหน่วยงาน, งานรออนุมัติ, ลูกทีมลาวันนี้, สถิติทีม |
| HR | ภาพรวมคำขอลา, วันหยุด, ยอดวันลา |
| ผู้ดูแลระบบ | คำขอลา/คำขอยกเลิกใบลาสำหรับ monitor, ผู้ใช้งาน, หน่วยงาน, Audit Log, Health, Backup, LINE |
| ผู้บริหาร | คำขอลาของฉัน, คำขอยกเลิกใบลา, คิวอนุมัติ, กราฟแนวโน้ม, ภาพรวมทั้งโรงพยาบาล |
...
สำหรับ Director, Admin และ SuperAdmin ระบบจะแสดง Card `คำขอลาของฉัน` และ `คำขอยกเลิกใบลา` ในส่วนต้นของ Dashboard เพื่อให้ตรวจภาพรวมงานลาได้ก่อน แล้วจึงตามด้วย Card เชิงบริหารหรือ Card จัดการระบบ
...
| คำขอยกเลิกใบลา | จำนวนคำขอยกเลิกใบลาและจำนวนวันที่คืนยอดเมื่ออนุมัติครบ |
...
12. [การอนุมัติคำขอยกเลิกใบลา](#การอนุมัติคำขอยกเลิกใบลา)
...
คำขอยกเลิกใบลาเป็นเอกสารแยกจากใบลาเดิม ใช้เลขที่ `LVC-xxxx` และใช้สายอนุมัติเดียวกับระบบลา

1. เปิดเมนู `ระบบลา`
2. เลือก `คำขอยกเลิกใบลา` หรือเปิดจาก `งานรออนุมัติของฉัน`
3. เลือกรายการที่มีสถานะ `รออนุมัติ`
4. ตรวจสอบใบลาเดิม เลขที่คำขอเดิม ประเภทลา วันที่ลา จำนวนวันที่จะคืนยอด และเหตุผล
5. ตรวจสอบ timeline การอนุมัติ
6. กด `อนุมัติ`, `ไม่อนุมัติ` หรือ `ตีกลับรอแก้ไข`
7. หากอนุมัติครบทุกขั้น ระบบจะเปลี่ยนใบลาเดิมเป็น `ยกเลิกหลังอนุมัติ` และคืนยอดวันลา

Checklist สำหรับผู้อนุมัติคำขอยกเลิกใบลา:

- [ ] ตรวจสอบว่าใบลาเดิมเป็นใบลาที่อนุมัติแล้วจริง
- [ ] ตรวจสอบเหตุผลการขอยกเลิกใบลา
- [ ] ตรวจสอบจำนวนวันที่จะคืนยอด
- [ ] ตรวจสอบผลกระทบต่อเวร/ตารางงานของหน่วยงาน
- [ ] ระบุหมายเหตุเมื่อไม่อนุมัติหรือตีกลับ

> **Warning:** การอนุมัติคำขอยกเลิกใบลาจะมีผลต่อยอดวันลาคงเหลือหลังอนุมัติครบทุกขั้น ควรตรวจสอบข้อมูลก่อนกดยืนยัน
