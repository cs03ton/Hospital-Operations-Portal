    ...
    ILogger<AdminLineController> logger) : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<LineOperationsStatusResponse>>> GetOperationsStatus(CancellationToken cancellationToken)
    {
        ...
        var response = new LineOperationsStatusResponse(
            ...
            IsHttps(lineConfiguration.Endpoint),
            null,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var items = await query
            ...
            .Take(pageSize)
            .Select(item => new LineDeliveryLogResponse(
                item.Id,
                item.CreatedAt,
                item.RecipientUser != null ? item.RecipientUser.FullName : "-",
    ...
    private IReadOnlyList<LineChecklistItemResponse> BuildChecklist()
    {
        ...
        [
            ...
            new("Access Token Loaded", lineConfiguration.HasAccessToken, "ตั้งค่า Line__ChannelAccessToken ผ่าน Environment Variables หรือ Secret Manager"),
            ...
            new("Webhook Verified", IsHttps(lineConfiguration.Endpoint), "ตรวจสอบ webhook URL ใน LINE Developers Console")
        ];
    }
