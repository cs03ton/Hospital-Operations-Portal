...
public sealed class FleetOperationsController(AppDbContext db, IWorkflowTransitionService workflow, IDomainEventPublisher events, ILogger<FleetOperationsController> logger) : ControllerBase
{
    ...
    private Task Publish(string type, FleetRequest item, Guid actor, CancellationToken ct) => events.PublishAsync(new(type, "FLEET", "FleetRequest", item.Id, actor, HttpContext.TraceIdentifier, new { FleetRequestId = item.Id, item.RequestNo, item.Status, item.ReturnTarget, item.Destination, item.DepartureAt, item.ExpectedReturnAt }, []), ct);
