-- Backup first. Execute whole script with Stop on error. On failure: ROLLBACK.
-- Requires AddRepairManagement schema. No users or user-role bindings are created.
BEGIN;
...
INSERT INTO line_group_destinations(id,line_group_id,display_name,status,module,delivery_provider,
 ...
 ON CONFLICT DO NOTHING;
SELECT t.code,t.name,count(c.id) AS categories FROM repair_teams t LEFT JOIN repair_categories c ON c.team_code=t.code GROUP BY t.code,t.name;
COMMIT;
